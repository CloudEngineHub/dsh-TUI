...
export function createChannel(
  ...
  const state: ChannelState = {
    ...
    async buildSessionTree(): Promise<SessionTreeData | null> {
      ...
      // inherit cwd, so the family never crosses projects. Subagent child
      // sessions carry parentSession too, but they are delegation artifacts,
      // not rewind branches — exclude them from the family.
      ...
      for (const id of ordered) {
        ...
        if (id === currentId) {
          ...
          // can display.
          const events = liveTailWindow(liveEvents, remaining)
          ...
          if (events.length !== liveEvents.length) truncated = true
          familySessions.push({
            id,
        ...
        familySessions.push({ ...facts, events, live: false })
