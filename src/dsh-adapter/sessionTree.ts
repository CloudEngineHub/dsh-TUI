/**
 * Session family tree — the model behind the double-Esc tree panel
 ...
 * file shapes them. Rendering lives in components/SessionTreePanel.tsx.
 *
...
/** Filter modes cycled by `f` in the tree panel (pi parity, minus labels). */
...
export interface TreeEntry {
   ...
   *  boundary -1, "cannot rewind to the very first message"), so the panel
...
export interface SessionTreeData {
  ...
  /** Per-session display facts (branch-head labels in the panel). */
...
export interface FlatNode {
  ...
  /** True when this pass produced multiple roots (panel shifts display). */
...
/** Preview cap for one entry line (the panel truncates further to width). */
...
export function buildSessionTree(
  ...
  for (const session of sessions) {
    if (session.parentSession === undefined || !byId.has(session.parentSession)) continue
    const siblings = childrenOf.get(session.parentSession)
    if (siblings === undefined) childrenOf.set(session.parentSession, [session])
  ...
  if (liveChain !== undefined) {
    ...
    while (current?.parentSession !== undefined && byId.has(current.parentSession)) {
  ...
  return { roots, activePath, activeLeafId, sessions: metas, truncated, sessionCount: sessions.length }
}
