...
export function SessionTreePanel({
  ...
  if (confirm !== null) {
    const cross = confirm.sessionId !== currentSessionId
    return (
      <Pane color="permission">
        <Box flexDirection="column">
          <Box marginBottom={1}>
            <Text color="remember" bold>
              {t('tree-confirm-title')}
            </Text>
            <Text dimColor>
              {cross
                ? t('tree-confirm-cross', { id: confirm.sessionId.slice(0, 8) })
                : t('tree-confirm-here')}
            </Text>
          </Box>
          <ListItem isFocused={false} description={t(`tree-kind-${confirm.kind}`)}>
            {truncateWidth(confirm.text, Math.max(16, columns - 10))}
          </ListItem>
